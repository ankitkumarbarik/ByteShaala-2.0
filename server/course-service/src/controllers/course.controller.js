...
export const updateCourse = asyncHandler(async (req, res) => {
  ...
  if (req.file?.buffer || thumbnailUrl) {
    ...
    if (existedCourse.thumbnailPublicId) {
      await deleteFromCloudinary(existedCourse.thumbnailPublicId, "image");
    }
...
export const deleteCourse = asyncHandler(async (req, res) => {
  ...
  await axios.patch(
    `${process.env.USER_SERVICE}/api/v1/user/remove-enrolled-course`,
    { courseId },
    { headers: { "x-user-data": req.headers["x-user-data"] } }
  );

  await axios.patch(
    `${process.env.CART_SERVICE}/api/v1/cart/remove-course`,
    { courseId },
    { headers: { "x-user-data": req.headers["x-user-data"] } }
  );
  ...
  if (req.headers["x-user-data"]) {
    try {
      const { data } = await axios.get(
        `${process.env.USER_SERVICE}/api/v1/user/current-user`,
        {
          headers: {
